...
class BillRegisterController extends BaseController
{
    ...
    public function __construct()
    {
        $this->db = db_connect(); // Loading database
         // Instantiate Services class
        // $this->services = new \App\Controllers\Services();
    }
...
public function store_bill_register()
{
    ...
    $validation = \Config\Services::validation();
    $BillRegister = new BillRegisterModel();
    $LiveBillRegister = new LiveBillRegisterModel();
    ...
    $uid = $this->db->table("asitek_bill_register")
        ->where("compeny_id", $this->request->getVar("compeny_id"))
        ->countAllResults();
    $uidno = $uid + 1;

    $vdr = $vendormodel->find($this->request->getVar("Vendor_Id"));
    $cpn = $companymodel->find($this->request->getVar("compeny_id"));

    $Gate_Entry_Date = date("Y-m-d", strtotime($this->request->getVar("Gate_Entry_Date")));
    $DateTime = date("Y-m-d H:i:s");

    $billduplicacy = $BillRegister->where("Vendor_Id", $this->request->getVar("Vendor_Id"))
        ->where("Bill_No", $this->request->getVar("Bill_No"))
        ->findAll();

    if (!empty($billduplicacy)) {
        $session->setFlashdata("success", "<div class='alert alert-danger'>Bill No already added</div>");
        return redirect()->to("add_bill_register");
    }
    ...
    $BillRegisterinsert = $BillRegister->insert($data);
    $LiveBillRegister->insert($data);

    if ($BillRegisterinsert) {
        $lastId = $BillRegister->getInsertID();

        // Reward Point Logic
        $Add_RewardPoint = $BillRegisterinsert > 0 ? 30 : 0;
        $Remark_RewardPoint = !empty($this->request->getVar("Remark")) ? 20 : 0;
        $FileAttch_RewardPoint = $file ? 30 : 0;
        ...
        if ($month <= $month2) {
            ...
            // Get current day and adjust logic
            $currentDay = date('d');
            if ($currentDay <= 10) {
                $reward_point = $Add_RewardPoint + $Remark_RewardPoint + $FileAttch_RewardPoint;
            } else {
        ...
        $session->setFlashdata("success", "<div class='alert alert-success'>Form Submission Successful. Bill ID: $uidno</div>");
        return redirect()->to("add_bill_register");
    } else {
        $session->setFlashdata("success", "<div class='alert alert-danger'>Problem in Submission!</div>");
        return redirect()->to("add_bill_register");
    }
}
...
public function update_bill_register()
{
    ...
    // Load both models
    $BillRegister = new BillRegisterModel(); // Local database
    $LiveBillRegister = new LiveBillRegisterModel(); // Live database

    // Get the record ID to update
    $recordId = $this->request->getVar("id");

    // Update both databases
    $localUpdate = $BillRegister->update($recordId, $data);
    $liveUpdate = $LiveBillRegister->update($recordId, $data);

    // Set a session flash message based on the update results
    $session = Services::session();
    if ($localUpdate && $liveUpdate) {
        $session->setFlashdata("success", "Record updated successfully in both local and live databases.");
    } else {
        $session->setFlashdata("error", "Failed to update record in one or both databases.");
    }

    // Redirect to the view page
    return redirect()->to("view_bill_register");
}
...
public function Bill_Acceptation_StatusChange()
{
    ...
    // Local and live models
    $model = new BillRegisterModel(); // Local
    $liveModel = new LiveBillRegisterModel(); // Live
    ...
    // Set current time and status by master ID
    date_default_timezone_set("Asia/Kolkata");
    $Bill_Acceptation_DateTime = date("Y-m-d H:i:s");
    $Bill_Acceptation_Status_By_MasterId = ($Bill_Acceptation_Status == 2 || $Bill_Acceptation_Status == 3)
        ? $result["emp_id"]
        : '';
    ...
    // Check the bill ID in local DB
    $billid = $this->db
        ->query("SELECT uid FROM asitek_bill_register WHERE id='$id' AND Bill_Acceptation_Status=1")
        ->getResult();

    if (isset($billid) && !empty($billid)) {
        // Update both local and live DBs
        $localUpdate = $model->where("id", $id)->set($data)->update();
        $liveUpdate = $liveModel->where("id", $id)->set($data)->update();

        if ($localUpdate && $liveUpdate) {
            // Reward point logic
            $Add_RewardPoint = 20;
            $Remark = $this->request->getVar("Bill_Acceptation_Status_Comments");
            $Remark_RewardPoint = $Remark ? 20 : 0;

            $BillRegrow = $model->where("id", $id)->first();
            ...
            // Reward point calculation based on the month and current day
            if ($month <= $month2) {
                // If current month is equal or before the bill month
                $reward_point = $Add_RewardPoint + $Remark_RewardPoint;
            } else {
                // If current month is after the bill month
                $currentDay = date('d');
                if ($currentDay <= 10) {  
                    $reward_point = $Add_RewardPoint + $Remark_RewardPoint; 
                } else {
            ...
            $reward_point_type = "Accept For Bill Assignment";
            $session_compeny_id = $session->get("compeny_id");
        ...
        // Bill ID not found or already processed
        $session->setFlashdata("Bill_Acceptation_Status", 2);
        return redirect("all_bill_mapping_list");
    }
}
...
public function Department_Mapping_BillReg()
{
    ...
    $result = $session->get();
    $email = \Config\Services::email();
    $model = new BillRegisterModel(); // Local model
    $livemodel = new LiveBillRegisterModel(); // Live model
    $vendormodel = new PartyUserModel();
    $companymodel = new CompenyModel();
    ...
    $data = [
        ...
        "TargetMapping_Time_Hours" => $this->request->getVar("TargetMapping_Time_Hours"),
        "Mapping_Delay_On_Time" => $this->request->getVar("Mapping_Delay_On_Time"),
        "Mapping_Remark" => $this->request->getVar("Mapping_Remark"),
    ];

    $billid = $this->db->query("SELECT id, compeny_id, Vendor_Id FROM asitek_bill_register WHERE id = '$id'")->getResult();
    if (empty($billid)) {
    ...
    $billidCheck = $this->db->query("SELECT uid FROM asitek_bill_register WHERE id='$id' AND Bill_Acceptation_Status=2")->getResult();
    if (!empty($billidCheck)) {
        // Update local and live models
        $localUpdate = $model->where("id", $id)->set($data)->update();
        $liveUpdate = $livemodel->where("id", $id)->set($data)->update();
        ...
        if ($localUpdate && $liveUpdate) {
            ...
            // Apply reward points calculation based on the current day and month
            if ($month <= $month2) {
                $reward_point = $Add_RewardPoint + $Remark_RewardPoint;
            } else {
                if ($currentDay <= 10) {
            ...
            $wpmsg = "Hello " . $vdr["Name"] . ", Just a quick update: going forward, please ensure that all your bills are mapped to " . $emp["first_name"] . " in the " . $dep["name"] . " department. Best regards, " . $cpn["name"];
            
            // Uncomment the following code to send WhatsApp message (if needed)
            ...
            $email->setSubject("Bill Added to Company | Bill Management");
            $email->setmailType("html");
            $email->setMessage($wpmsg); // The message here
            $email->send();
            ...
            $session->setFlashdata("Mapping_Department", 2);
            return redirect("all_bill_mapping_list");
        }
    } else {
        $session->setFlashdata("Mapping_Department", 3);
...
public function Clear_Bill_Form_StatusChange()
{
    ...
    $MasterId_By_emp_id = ($status == 2 || $status == 3) ? $result["emp_id"] : "";

    $data = [
        "MasterId_By_Clear_Bill_Form" => $MasterId_By_emp_id,
        "Clear_Bill_Form_Status" => $this->request->getVar("Clear_Bill_Form_Status"),
        "Clear_Bill_Form_DateTime" => $Clear_Bill_Form_DateTime,
        "Clear_Bill_Form_Status_Comments" => $this->request->getVar("Clear_Bill_Form_Status_Comments"),
    ];

    $billid = $this->db
        ->query("SELECT uid FROM asitek_bill_register WHERE id='$id' AND Clear_Bill_Form_Status=1")
        ->getResult();

    if (isset($billid) && !empty($billid)) {
        $localUpdate = $model->where("id", $id)->set($data)->update(); // Update local database
        $liveUpdate = $livemodel->where("id", $id)->set($data)->update(); // Update live database
        ...
        if ($localUpdate && $liveUpdate) {
            ...
            // Current day logic
            $currentDay = date('d'); // Get the current day of the month (01 to 31)

            if ($month <= $month2) {
                $reward_point = $Add_RewardPoint + $Remark_RewardPoint;
            } else {
                // Adjust reward points based on the current day
                if ($currentDay <= 10) {
                    $reward_point = $Add_RewardPoint + $Remark_RewardPoint; // First 10 days
                } else {
                    $reward_point = $Add_RewardPoint + $Remark_RewardPoint; // After 10th day
                }
            }

            $reward_point_type = "Accept For Bill Verify";
            $session_compeny_id = $session->get("compeny_id");
            $session_emp_id = $session->get("emp_id");
            $DateTimenew = date("Y-m-d H:i:s");
            $dataRewardPoint = [
...
{
    // Get session data and initialize models
    $session = \Config\Services::session();
    $result = $session->get();
    $email = \Config\Services::email();
    
    // Models for local and live databases
    $model = new BillRegisterModel();  // Local DB model
    $livemodel = new LiveBillRegisterModel();  // Live DB model
    ...
    $data = [
        ...
        "TargetClearBillForm_Time_Hours" => $this->request->getVar("TargetClearBillForm_Time_Hours"),
        "ClearBillForm_Delay_On_Time" => $this->request->getVar("ClearBillForm_Delay_On_Time"),
        "Clear_Bill_Form_AnyImage" => $imageName,
        "ClearBillForm_Remark" => $this->request->getVar("ClearBillForm_Remark"),
    ];

    // Fetch vendor and company details
    $billid = $this->db->query("SELECT compeny_id, Vendor_Id from asitek_bill_register WHERE id = '$id'")->getResult();
    $vendorvalue = $billid[0]->Vendor_Id;
    $compnyvalue = $billid[0]->compeny_id;
    ...
    // Check if the bill is ready for update
    $billmasterid = $this->db->query("SELECT uid from asitek_bill_register WHERE id='$id' AND Clear_Bill_Form_Status=2")->getResult();

    // Insert or update the record in both local and live databases
    if (!empty($billmasterid)) {
        // Local DB update
        $model->where("id", $id)->set($data)->update();

        // Live DB update
        $livemodel->where("id", $id)->set($data)->update();
        ...
        $FileAttch_RewardPoint = ($E_Image != "") ? 10 : 0;

        // Fetch the Bill's DateTime to compare months
        $BillRegObjNew = new BillRegisterModel();
        $BillRegrow = $BillRegObjNew->where('id', $id)->first();
        $Bill_DateTime2 = $BillRegrow['Bill_DateTime'];
        ...
        if ($month <= $month2) {
            // Same or earlier month, no day-based logic
            $reward_point = $Add_RewardPoint + $Remark_RewardPoint + $FileAttch_RewardPoint;
        } else {
            // If month is different, apply day logic
            $currentDay = date('d');  // Current day of the month

            if ($currentDay <= 10) {   
                // First 30 days of the month
                $reward_point = $Add_RewardPoint + $Remark_RewardPoint + $FileAttch_RewardPoint;
            } else {
                // After 30th day of the month
                $reward_point = $Add_RewardPoint + $Remark_RewardPoint + $FileAttch_RewardPoint;
            }
        }
        ...
        // Insert Reward Point into the local database
        $RewardPointobj = new RewardPointModel();
        $RewardPoint = $RewardPointobj->insert($dataRewardPoint);

        // Local and live database insertion (both)
        $localModel = new BillRegisterModel();
        $localModel->insert($data);  // Insert to local database
        
        // $liveModel = new LiveBillRegisterModel();
        // $liveModel->insert($data);  // Insert to live database

        // Email or SMS Notification (Commented out for now, but left in place)
        // $wpmsg = "Dear " . $vdr["Name"] . ", Your bill has been forwarded to ERP system.";
        // $email->setFrom("singhshaan085@gmail.com", "Bill Management");
        // $email->setTo($vdr["Email_Id"]);
        // $email->setSubject("Bill Add in Company | Bill Management");
        // $email->setmailType("html");
        // $email->setMessage($wpmsg);
        // $email->send();
    ...
    public function MasterAction_send()
    {
        ...
        $compeny_id = $session->get("compeny_id");
        $model = new BillRegisterModel();
        $livemodel = new LiveBillRegisterModel(); // Added for live DB
        $MasterActionmadelObj = new MasterActionModel();
        $MasterActionUploadModelObj = new MasterActionUploadModel();
        $id = $this->request->getVar("id");
        ...
        if (isset($billid) && !empty($billid)) {
            ...
            if(isset($MasterActionUploadrow333) && $MasterActionUploadrow333!='')
            {
                ...
                if ($insert) {
                    ...
                    $livemodel->where("id", $id)->set("ClearFormBill_Master_Action", $rowMasterAction1["no_of_action"])->update(); // Update live DB
                    // $model->where("id", $id)->set("ClearFormBill_Master_Action", $rowMasterAction1["no_of_action"])->update(); // Update local DB
                        // ****Start RewardPoint**
                        $MasterActionmadelObj = new MasterActionModel();
                        ...
                        $model->where("id", $id)->set("ERP_Master_Action",$rowMasterAction1["no_of_action"])->update();
                         ...
                         $livemodel->where("id", $id)->set("ERP_Master_Action", $rowMasterAction1["no_of_action"])->update(); // Update live DB
                    // $model->where("id", $id)->set("ERP_Master_Action", $rowMasterAction1["no_of_action"])->update(); // Update local DB
                        // ****Start RewardPoint**
                        ...
                        $livemodel->where("id", $id)->set("Recived_Master_Action", $rowMasterAction1["no_of_action"])->update(); // Update live DB
                    // $model->where("id", $id)->set("Recived_Master_Action", $rowMasterAction1["no_of_action"])->update(); // Update local DB
                        ...
                        if (isset($rowMasterAction1) && $rowMasterAction1 != "") {
                            if ($rowMasterAction1["id"] == $Master_ActionId) {
                                ...
                                else
                                {
                                    $currentDay = date('d');
                                    if ($currentDay <= 10) {   
                                        $reward_point =$reward_point =  $Add_RewardPoint + $Remark_RewardPoint +$FileAttch_RewardPoint;
                                    }
                                    else{
                                        $reward_point =$reward_point =  $Add_RewardPoint + $Remark_RewardPoint +$FileAttch_RewardPoint;
                                    }
...
public function ERP_StatusChange()
{
    ...
    // Set the ERP_Status_Change_By_MasterId based on ERP_Status
    $ERP_Status_Change_By_MasterId = ($ERP_Status == 2 || $ERP_Status == 3) ? $result["emp_id"] : '';
    ...
    // Query the bill register table to fetch the bill ID
    $billid = $this->db->query("SELECT uid from asitek_bill_register WHERE id='$id' AND ERP_Status=1")->getResult();

    if (isset($billid) && !empty($billid)) {
        // Update data in both models (local and live)
        $updateLocal = $model->where("id", $id)->set($data)->update();
        $updateLive = $livemodel->where("id", $id)->set($data)->update();

        // Check if update was successful in both models
        if ($updateLocal && $updateLive) {
            // Reward Point Logic
            $Add_RewardPoint = 10;
            ...
            // Month-based reward point logic
            if ($month <= $month2) {
                $reward_point = $Add_RewardPoint + $Remark_RewardPoint;
            } else {
                // Current Day Logic: Reward points based on the current day of the month
                $currentDay = date('d');
                if ($currentDay <= 10) {
                    // If current day is 10 or before, award full reward points
                    $reward_point = $Add_RewardPoint + $Remark_RewardPoint;
                } else {
                    // After 10th, still award full reward points, but you can adjust the logic here if needed
                    $reward_point = $Add_RewardPoint + $Remark_RewardPoint;
                }
            }
...
public function CheckUp_bill_ERPSytem()
{
    ...
    $data = [
        ...
        "ERP_Status" => 4,
        "Target_ERP_Time_Hours" => $this->request->getVar("Target_ERP_Time_Hours"),
    ...
    $billidStatus = $this->db
        ->query(
            "SELECT uid FROM asitek_bill_register WHERE id='$id' AND ERP_Status=2"
        )
        ->getResult();

    if (isset($billidStatus) && !empty($billidStatus)) {
        // Perform updates for both local and live models
        if ($model->where("id", $id)->set($data)->update() && $livemodel->where("id", $id)->set($data)->update()) {
            // Reward Point Logic
            $Add_RewardPoint = 10;
            $Remark_RewardPoint = !empty($data["ERP_Remark"]) ? 10 : 0;
            $FileAttch_RewardPoint = !empty($data["ERP_AnyImage"]) ? 10 : 0;
...
public function RecivedBill_StatusChange()
{
    ...
    // Set the Recived_Status_Change_By_MasterId based on Recived_Status
    $Recived_Status_Change_By_MasterId = ($Recived_Status == 2 || $Recived_Status == 4) 
        ? $result["emp_id"] 
        : '';
    ...
    // Query the bill register table to fetch the bill ID
    $billid = $this->db->query("SELECT uid FROM asitek_bill_register WHERE id='$id' AND Recived_Status=1")->getResult();
    ...
    if (isset($billid) && !empty($billid)) {
        ...
        if ($updateLocal && $updateLive) {
            ...
            if ($month <= $month2) {
                ...
                $currentDay = date('d');
                $reward_point = ($currentDay <= 10) 
                    ? $Add_RewardPoint + $Remark_RewardPoint 
                    : $Add_RewardPoint + $Remark_RewardPoint;
            }

            $reward_point_type = "Accept For Bill Received A/C";
            $session_compeny_id = $session->get("compeny_id");
            $session_emp_id = $session->get("emp_id");
            $DateTimenew = date("Y-m-d H:i:s");
...
public function CheckUp_RecivedBill()
{
    ...
    $billid = $this->db
        ->query("SELECT compeny_id, Vendor_Id FROM asitek_bill_register WHERE id = '$id'")
        ->getResult();
    ...
    $billid = $this->db
        ->query("SELECT uid FROM asitek_bill_register WHERE id = '$id' AND Recived_Status = 2")
        ->getResult();

    if (isset($billid) && !empty($billid)) {
        // Update both local and live databases
        $localUpdate = $model->where("id", $id)->set($data)->update();
        $liveUpdate = $livemodel->where("id", $id)->set($data)->update();
        ...
        if ($localUpdate && $liveUpdate) {
            ...
            $id3 = $this->request->getVar("id");
            $BillRegrow = $model->where("id", $id3)->first();
            $Bill_DateTime2 = $BillRegrow["Bill_DateTime"];
            $currentmonth = date("Y-m-d H:i:s");
