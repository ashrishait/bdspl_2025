...
public function store_bill_register()
    ...
    //     $BillRegister = new BillRegisterModel();
    //     if ($BillRegister->update($this->request->getVar("id"), $data)) {
    //         $session = Services::session();
...
public function Department_Mapping_BillReg()
{
    ...
    $billid = $this->db->query("SELECT uid FROM asitek_bill_register WHERE id='$id' AND Bill_Acceptation_Status=2")->getResult();
    if (isset($billid) && !empty($billid)) {
        ...
        if ($localUpdate && $liveUpdate) {
            ...
            $month = date('m', strtotime($currentmonth));
            $month2 = date('m', strtotime($Bill_DateTime2));
            $reward_point = ($month <= $month2) ? $Add_RewardPoint + $Remark_RewardPoint : $Add_RewardPoint + $Remark_RewardPoint;
...
public function Clear_Bill_Form_StatusChange()
{
    ...
    if (isset($billid) && !empty($billid)) {
        ...
        if ($localUpdate && $liveUpdate) {
            ...
            $reward_point = ($month <= $month2) ? 
                $Add_RewardPoint + $Remark_RewardPoint : 
                (($currentDay = date('d')) <= 10 ? 
                    $Add_RewardPoint + $Remark_RewardPoint : 
                    $Add_RewardPoint + $Remark_RewardPoint);
...
public function MasterAction_send()
{
    ...
    // Use both models - for local and live databases
    $model = new BillRegisterModel();
    ...
    // Handle different stages with conditions based on stage_id
    if ($stage_id == 3) {
        $billid = $this->db->query("SELECT id from asitek_bill_register WHERE id = '$id' AND Clear_Bill_Form_Status=2")->getResult();
    } elseif ($stage_id == 4) {
        $billid = $this->db->query("SELECT id from asitek_bill_register WHERE id = '$id' AND Clear_Bill_Form_Status=4 AND ERP_Status=2")->getResult();
    ...
    // print_r($stage_id);
    // print_r($Master_ActionId);
    // print_r($remark);
    // print_r($action);
    // exit();

    if (isset($billid) && !empty($billid)) {
        // Validate file if it's uploaded
        if ($file != "") {
            $validation->setRules([
                "E_Image" => "uploaded[E_Image]|ext_in[E_Image,jpg,JPG,png,PNG,jpeg,JPEG,pdf]",
            ]);
        ...
        if (isset($MasterActionUploadrow333) && $MasterActionUploadrow333 != '') {
            ...
            if ($insert) {
                ...
                if ($stage_id == 3) {
                    // Update local and live databases
                    $model->where("id", $id)->set("ClearFormBill_Master_Action", $rowMasterAction1["no_of_action"])->update();
                    $livemodel->where("id", $id)->set("ClearFormBill_Master_Action", $rowMasterAction1["no_of_action"])->update();
                    
                    // Reward point logic for local and live updates
                    $this->rewardPointLogic($id, $rowMasterAction1, $session, $remark, $file);
                    ...
                    if ($action == "all") {
                        ...
                        echo "<script type='text/javascript'> alert('Successfully '); window.location.href='" . base_url("/index.php/sigle_bill_list/" . $id) . "'</script>";
                    }
                } elseif ($stage_id == 4) {
                    // Similar update for stage 4
                    $model->where("id", $id)->set("ERP_Master_Action", $rowMasterAction1["no_of_action"])->update();
                    $livemodel->where("id", $id)->set("ERP_Master_Action", $rowMasterAction1["no_of_action"])->update();
                    ...
                    // Similar update for stage 5
                    $model->where("id", $id)->set("Recived_Master_Action", $rowMasterAction1["no_of_action"])->update();
                    $livemodel->where("id", $id)->set("Recived_Master_Action", $rowMasterAction1["no_of_action"])->update();
...
private function rewardPointLogic($id, $rowMasterAction1, $session, $remark, $file)
{
    // Reward points calculation
    $Add_RewardPoint = 10;
    $Remark_RewardPoint = !empty($remark) ? 10 : 0;
    $FileAttch_RewardPoint = $file ? 10 : 0;
...
public function CheckUp_Clear_Bill_Form()
{
    ...
    // Data for BillRegisterModel (same data for both models)
    $data = [
        "Mapping_ERP_EmpId" => $this->request->getVar("Mapping_ERP_EmpId"),
        "Mapping_ERP_EmpId_By_MasterId" => $Mapping_ERP_EmpId_By_MasterId,
        "Clear_Bill_Form_Status" => 4,
    ...
    if (isset($billmasterid) && !empty($billmasterid)) {
        // Update data in both models
        $updateLocal = $model->where("id", $id)->set($data)->update();
        $updateLive = $livemodel->where("id", $id)->set($data)->update(); // Update in Live model too
        ...
        if ($updateLocal && $updateLive) {
            ...
            if ($action == "all") {
                ...
                // Email/WhatsApp sending logic
                // Uncomment and adjust for real use case
                // $email->setFrom("singhshaan085@gmail.com", "Bill Management");
                // $email->setTo($vdr["Email_Id"]);
                // $email->setSubject("Bill Add in Company | Bill Management");
                // $email->setmailType("html");
    ...
    if (isset($billid) && !empty($billid)) {
        ...
        if ($updateLocal && $updateLive) {
            ...
            if ($month <= $month2) {
                ...
                $currentDay = date('d');
                $reward_point = ($currentDay <= 10) ? $Add_RewardPoint + $Remark_RewardPoint : $Add_RewardPoint + $Remark_RewardPoint;
            }
